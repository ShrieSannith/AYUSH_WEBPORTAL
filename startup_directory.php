...
<?php endif; ?> 
      ...
      <main role="main">
        ...
        <header class="grid">
            ...
            <h1 class="text-strong text-white text-center center text-size-60 text-uppercase margin-bottom-20">STARTUP DIRECTORY</h1>
            <p class="text-size-20 text-white text-center center">Welcome to Ayush Startup Directory. Here you will find the famous startups based on Medicine and fitness. If you have a startup, you can also register it in our "Register your startup" portal.</p>
          </div>
        </header>
        ...
        <section class="grid margin">
            <?php
               ...
               // Fetch startup data from the database
               ...
               if ($result && mysqli_num_rows($result) > 0) {
                   while ($row = mysqli_fetch_assoc($result)) {
                       ...
                       $mobileNumber = "+1 " . rand(100, 999) . "-" . rand(100, 999) . "-" . rand(1000, 9999);

                       echo "<div class='s-12 m-6 margin-bottom'>";
                       echo "<div class='startup-card'>";
                       echo "<div class='startup-content'>";
                       echo "<h2 class='startup-title'>{$row['Start_Up']}</h2>";
                       echo "<p class='startup-funding'>Funding: {$row['Funding']}</p>";
                       
                       echo "<p class='startup-description'>Description: {$row['Description']}</p>";
                       echo "<p class='startup-contact'>Contact: {$mobileNumber}</p>"; // Display the random contact number
                   ...
                   echo "<p class='no-startups'>No startups found.</p>";
               }
