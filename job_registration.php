...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    $query = "INSERT INTO jobs (job_title, company, location, salary, requirements, Number_Required, job_description) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $query);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "sssssis", $jobTitle, $company, $location, $salary, $requirements, $Number_Required, $jobDescription);
...
<head>
    ...
    <style>
        ...
        /* ... (Styles provided in the previous responses) ... */
        ...
        <div class="row">
            <div class="col-md-16">
                <main class="form-signin text-center">
                    <form action="job_registration.php" method="POST">
                        <h1 class="h3 mb-3 fw-normal">Give a Job Ad</h1>
                        <?php
                        ...
                        <div class="form-floating mb-3">
                            <textarea class="form-control" id="requirements" name="requirements" rows="4" placeholder="Requirements" required></textarea>
                            <label for="requirements">Requirements</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="number" class="form-control" id="Number_Required" name="Number_Required" placeholder="Number Required" required>
                            <label for="Number_Required">Number Required</label>
                        </div>
                        <div class="form-floating mb-3">
                            <textarea class="form-control" id="job_description" name="job_description" rows="4" placeholder="Job Description" required></textarea>
                            <label for="job_description">Job Description</label>
                        </div>
                        <button class="btn btn-primary w-30 py-2" type="submit" name="submit">Submit</button>
                    </form>
                </main>
            </div>
        </div>
